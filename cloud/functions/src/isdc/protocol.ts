...
async function handleDetailsSync(
  ...
  try {
    ...
    if (entities.inspections && entities.inspections.length > 0) {
      const inspectionResults = await syncInspections(
        entities.inspections,
        client_state.local_versions,
        userId,
        db
      );
    ...
    if (entities.findings && entities.findings.length > 0) {
      const findingResults = await syncFindings(
        entities.findings,
        client_state.local_versions,
        userId,
        db
    ...
    await logAuditRecord(
      ...
      'details.sync',
      'success',
      `Synced ${syncEntities.length} entities`
    );
...
async function syncInspections(
...
): Promise<{ entities: SyncEntity[]; conflicts: ConflictInfo[] }> {
  ...
  for (const inspectionId of inspectionIds) {
    try {
      ...
      const inspectionData = inspectionDoc.data() as InspectionDetails;
...
async function syncFindings(
  ...
  for (const findingId of findingIds) {
    try {
      ...
      const findingData = findingDoc.data() as FindingDetails;
...
async function handleInspectionOperation(
  ...
  const inspectionData: Partial<InspectionDetails> = {
    ...inspection,
    inspector_id: userId,
...
async function handleFindingOperation(
  ...
  const findingData: Partial<FindingDetails> = {
    ...finding,
    updated_at: new Date().toISOString(),
