...
function publicMessageFor(params: { verdict: BaneOutput['verdict']; enforcementLevel: number }): string | undefined {
  if (params.verdict === 'REFUSE' || params.verdict === 'REFUSE_AND_LOG' || params.verdict === 'REFUSE_CONTAIN_ESCALATE_TO_IU') {
  ...
  if (params.verdict === 'PAUSE_FOR_VERIFICATION') {
  ...
  return {
    evaluate(input: BaneInput): BaneOutput {
      ...
      if (pre) {
        ...
        if (pre.verdict === 'REFUSE' || pre.verdict === 'REFUSE_AND_LOG' || pre.verdict === 'REFUSE_CONTAIN_ESCALATE_TO_IU') {
      ...
      let enforcementLevel: BaneOutput['enforcementLevel'] =
        agg.verdict === 'REFUSE' || agg.verdict === 'REFUSE_AND_LOG' || agg.verdict === 'REFUSE_CONTAIN_ESCALATE_TO_IU'
          ? 3
          : agg.verdict === 'PAUSE_FOR_VERIFICATION'
            ? 2
            : agg.verdict === 'DOWNSCOPE'
              ? 1
              : 0;
      ...
      const safeText =
        (agg.verdict === 'DOWNSCOPE' || agg.verdict === 'PAUSE_FOR_VERIFICATION') && spans.length
