# Dual-Repo Portability Recovery Manifest
...
## Domains Targeted for Recovery
1. **NOAA Weather Federations:** `src/app/api/weather`, providers, normalizers
2. **Shell Contract Artifacts:** Legacy or unmapped TopCommandBar/OverHUD/Shell structural elements
3. **Inspections Stack:** Missing page components and flow logic in `src/app/(authenticated)/inspections`
4. **Contractor Stack:** Missing components for `PartyIntake` and workflow flows for contractors
5. **Recognition Stack:** Missing engine configurations and rule definitions without overriding current type hardening
