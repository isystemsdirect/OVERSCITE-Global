        ...
        <div className="lg:col-span-2 space-y-6">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <div className="mt-6">
                <TabsContent value="flight-controller">
                    <FlightControllerConfig droneId={droneId} config={config.flightController} onChange={(fc) => setConfig({...config, flightController: fc})} />
                </TabsContent>
                <TabsContent value="ecu">
                    <ECUManagement 
            droneId={droneId} 
            ecuData={{ rpm: 0, temp: 0 }} 
            onECUCommand={(cmd) => {
              console.log('ECU Command:', cmd);
          ...
          />
                ...
                <TabsContent value="sensors">
                    <SensorDataPane 
                        droneId={droneId} 
                        sensorData={config.sensors} 
                        onSensorToggle={(id, val) => console.log(`Sensor ${id} toggle: ${val}`)} 
                    />
                </TabsContent>
