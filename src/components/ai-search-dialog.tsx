...
export function AiSearchDialog() {
  ...
  const processAndRespond = useCallback(async (text: string) => {
    ...
    toast({
      ...
      description: `The ${CANON.BFI} features are currently unavailable due to dependency issues.`,
    });
    setDialogState('idle');
  }, [toast]);
  ...
  async function onSubmit(values: z.infer<typeof searchSchema>) {
    ...
    try {
      ...
      // setResults(searchResults);
       toast({
        variant: "destructive",
        title: `${CANON.BFI} Search Disabled`,
        description: `${CANON.BFI} features are temporarily disabled.`
      })
      setResults({ citations: [] });

    } catch (error) {
      console.error(`${CANON.BFI} Search failed:`, error);
      toast({
        variant: "destructive",
        title: "Search Failed",
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogTrigger asChild>
        <Button
          variant="outline"
          className="w-full justify-start text-sm text-muted-foreground rounded-xl scing-border-glow bg-background/30 hover:bg-background/50 hover:border-primary/40 transition-all duration-500 group"
        >
      ...
      <DialogContent className="sm:max-w-[625px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {isVisualSearchActive ? "Position the subject in the frame and capture." : `Cross-check standards, ask questions, or issue voice commands to your ${CANON.INTELLIGENCE_TYPE} model.`}
          </DialogDescription>
        </DialogHeader>
                      ...
                      <FormControl>
                        <div className="relative">
                          ...
                          <Input
                            ...
                            className={cn("pr-20 rounded-xl border-border/50", capturedImage ? "pl-12" : "pl-9")}
                            {...field}
                          />
                  ...
                  {results.citations && results.citations.length > 0 ? (
                    ...
                    <div className="text-center p-8 border rounded-lg">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">
                          The {CANON.BFI} couldn't find any matching standards for your query. Try rephrasing your search or uploading more documents to your library.
                        </p>
                    </div>
                  )}
                </div>
